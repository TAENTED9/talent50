import { Button } from './ui/Button';

export default function Hero() {
  return <section id="home" className="relative flex min-h-screen items-end overflow-hidden bg-ink pb-[clamp(48px,7vh,96px)]">
    <div className="absolute inset-[-12%_0] bg-[#121310] grayscale-[.25]" /><div className="absolute inset-0 bg-hero-overlay" />
    <div className="relative z-10 mx-auto flex h-[530px] w-full max-w-content animate-fade-up flex-col justify-end px-5 md:px-[5vw] lg:px-[72px]">
      <div className="mb-[26px] text-xs uppercase tracking-[0.16em] text-text-light/60">Talent50 Group</div>
      <h1 className="mb-[30px] max-w-[17ch] font-display text-[clamp(42px,7vw,110px)] font-bold leading-[.98] tracking-[-0.02em] text-text-light">Building the <span className="text-accent">missing middle</span> of African sport.</h1>
      <p className="mb-10 max-w-[58ch] text-[clamp(17px,1.4vw,21px)] font-light leading-[1.55] text-muted-light">The Talent50 Group unlocks the value of African sports, turning potential into assets.</p>
      <div className="flex flex-wrap items-center gap-[18px]"><Button href="#contact">Partner with us</Button><Button href="#engine" variant="ghost">See how it works</Button></div>
    </div><a href="#about" className="absolute bottom-8 right-5 z-10 text-xs uppercase tracking-[0.16em] text-text-light/60 transition-colors hover:text-accent focus-visible:outline focus-visible:outline-[3px] focus-visible:outline-accent focus-visible:outline-offset-2">Scroll <span className="ml-2 inline-block animate-float text-lg">↓</span></a>
  </section>;
}
