...
export function SectionHeading({ eyebrow, title, body, theme = 'dark', titleClassName = '', bodyClassName = '' }: SectionHeadingProps) {
  ...
  return <>
    <div className={`mb-[26px] font-mono text-[11px] uppercase tracking-[0.14em] ${isDarkSection ? 'text-text-light/50' : 'text-ink/50'}`}>{eyebrow}</div>
    <h2 className={`mb-6 max-w-[20ch] font-display text-[clamp(34px,5vw,72px)] leading-[1.04] tracking-[-0.02em] ${isDarkSection ? 'text-text-light' : 'text-ink'} ${titleClassName}`}>{title}</h2>
